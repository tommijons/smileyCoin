...
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    if (strMethod == "consolidate"            && n > 1) ConvertTo<int64_t>(params[1]);
    ...
    if (strMethod == "sendtotom"              && n > 0) ConvertTo<double>(params[0]);
    ...
    //if (strMethod == "replywithmessage"       && n > 1) ConvertTo<double>(params[1]);
